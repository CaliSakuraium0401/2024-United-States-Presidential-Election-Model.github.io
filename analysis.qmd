...
|------|:--------------|------|:--------------|:--------------|
| ![](images/Flag_Pennsylvania.png) | Pennsylvania | 19 | `r pa_harris` % | `r pa_trump` % |
| ![](images/Flag_Michigan.png) | Michigan | 5 | `r mi_harris` % | `r mi_trump` % |
| ![](images/Flag_Wisconsin.png) | Wisconsin | 10 | `r wi_harris` % | `r wi_trump` % |
| ![](images/Flag_NorthCarolina.png) | North Carolina | 16 | `r nc_harris` % | `r nc_trump` % |
| ![](images/Flag_Georgia.png) | Georgia | 16 | `r ga_harris` % | `r ga_trump` % |
| ![](images/Flag_Arizona.png) | Arizona | 11 | `r az_harris` % | `r az_trump` % |
| ![](images/Flag_Nevada.png) | Nevada | 6 | `r nv_harris` % | `r nv_trump` % |
...
favorability_polling <- favorability_polling_ut |>
    select(pollster, sponsors, numeric_grade, pollscore, 
    transparency_score, start_date, end_date, sample_size,
    population, internal, partisan, politician, favorable:very_unfavorable) |>
    filter(
        !is.na(favorable),
        !is.na(unfavorable),
        ...
        !pollster %in% c("Trafalgar Group", "Patriot Polling", "SoCal Strategies", "RMG Research", "InsiderAdvantage", "co/efficient"),
        internal != "TRUE",
        !partisan %in% c("DEM", "REP"),
        politician %in% c("Donald Trump", "JD Vance", "Kamala Harris", "Tim Walz")
    ) |>
    mutate(
        start_date = as.Date(start_date, format="%m/%d/%y"),
        end_date = as.Date(end_date, format="%m/%d/%y")
...
favorability_polling_duo_summary <- favorability_polling_duo |>
    filter(!is.na(sample_size)) |>
    summarize(
        pct = weighted.mean(pct, sample_size),
        .by = c(start_date, politician, view, pct)
    ) |> 
    arrange(start_date)
...
favorability_polling_scale_summary <- favorability_polling_scale |>
    filter(!is.na(sample_size)) |>
    summarize(
        pct = weighted.mean(pct, sample_size),
        .by = c(start_date, politician, view, pct)
